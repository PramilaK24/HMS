...
export default function ValidationControl() {
  ...
  return (
    <div className="h-fit rounded-lg border border-text-accent/60 bg-[#08170f] p-5 shadow-[inset_0_0_14px_#00a04812]">
      ...
      <ul className="mt-4 space-y-3.5 text-xs text-white/85">
        {rules.map((rule, idx) => (
          <li key={idx} className="flex items-center gap-3">
            <div className="flex size-7 items-center justify-center shrink-0  text-white/80">
              <Icon icon={rule.icon} width="19" aria-hidden="true" />
            </div>
